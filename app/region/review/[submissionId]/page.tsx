...
import { approveRegionalSubmission, returnRegionalSubmission } from "../actions";

export default async function RegionReviewDetailPage({
  params,
}: {
  params: Promise<{ submissionId: string }>;
}) {
  await requireRole(["rvp"]);
  ...
  const isPending = submission.regional_review_status === "pending";
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <ReviewActionPanel
        approveAction={approveRegionalSubmission.bind(null, submission.id)}
        returnAction={returnRegionalSubmission.bind(null, submission.id)}
        disabled={!isPending}
        disabledReason={`Regional review already ${submission.regional_review_status}.`}
      />
    </div>
  );
}
